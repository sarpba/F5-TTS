...
# Function to load F5-TTS model
def load_f5tts(ckpt_path):
    F5TTS_model_cfg = dict(dim=1024, depth=22, heads=16, ff_mult=2, text_dim=512, conv_layers=4)
    return load_model(DiT, F5TTS_model_cfg, ckpt_path)

# Function to load E2-TTS model
def load_e2tts(ckpt_path):
...
@click.option("--ckpt", default=None, type=str, help="Path to the F5-TTS model checkpoint")
@click.option("--vocab", default=None, type=str, help="Path to the vocab.txt file")
def main(port, host, share, api, ckpt, vocab):
    global F5TTS_ema_model, E2TTS_ema_model, custom_ema_model, pre_custom_path, tts_model_choice, vocoder
    ...
    # Flag to indicate if custom model is used
    custom_model_used = False

    # Handle custom model paths if provided
    if ckpt and vocab:
        tts_model_choice = ["Custom", ckpt, vocab]
        print(f"Loading custom F5-TTS model from {ckpt} with vocab {vocab}")
        custom_ema_model = load_custom(ckpt, vocab_path=vocab)
        pre_custom_path = ckpt
        custom_model_used = True
    else:
        # Load default F5-TTS model
        default_ckpt = "hf://SWivid/F5-TTS/F5TTS_Base/model_1200000.safetensors"
        print(f"Loading default F5-TTS model from {default_ckpt}")
        F5TTS_ema_model = load_f5tts(default_ckpt)

        # Load E2-TTS model only if custom model is not used
        if not custom_model_used and USING_SPACES:
            E2TTS_ckpt = "hf://SWivid/E2-TTS/E2TTS_Base/model_1200000.safetensors"
            print(f"Loading default E2-TTS model from {E2TTS_ckpt}")
            E2TTS_ema_model = load_e2tts(E2TTS_ckpt)

    # Define the Gradio app inside main to utilize loaded models
...
""")
        ...
        with chat_interface_container:
            ...
            def generate_audio_response(history, ref_audio, ref_text, remove_silence):
                ...
                if not history or not ref_audio:
                    return None, gr.update(value=ref_text)
            ...
            audio_input_chat.stop_recording(
                ...
                outputs=[chatbot_interface, conversation_state],  # Kijavítva
            ).then(
    ...
    **NOTE: Reference text will be automatically transcribed with Whisper if not provided. For best results, keep your reference clips short (<15s). Ensure the audio is fully uploaded before generating.**
    """
        ...
        with gr.Row():
            if not custom_model_used:
                # Only show E2-TTS option if custom model is not used
                if not USING_SPACES:
                    choices = [DEFAULT_TTS_MODEL, "E2-TTS", "Custom"]
                else:
                    choices = [DEFAULT_TTS_MODEL, "E2-TTS"]
            else:
                # When custom model is used, only allow F5-TTS
                choices = [DEFAULT_TTS_MODEL, "Custom"]

            choose_tts_model = gr.Radio(
                choices=choices, 
                label="Choose TTS Model", 
                value=DEFAULT_TTS_MODEL if not custom_model_used else "Custom"
            )
